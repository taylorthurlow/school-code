public class Main {
    ...
    public static void main(String[] args) {
        ...
        int iteration = 0;
        double xi = 150;
        double xip1 = xi - function(function, xi) * ((delta * xi) / (function(function, (xi + delta * xi)) - function(function, xi)));
        double err = 999999.0;

        printSecantRowFunction(function, iteration, xi, xip1, 0);

        while (!toleranceSatisfied(xip1, xi, delta) && iteration < 100) {
            // store xip1 value for err calc
            double prev_xip1 = xip1;

            iteration++;
            xi = xip1;
            xip1 = xi - function(function, xi) * ((delta * xi) / (function(function, (xi + delta * xi)) - function(function, xi)));
            err = Math.abs((xip1 - prev_xip1) / xip1);

            printSecantRowFunction(function, iteration, xi, xip1, err);
        }
    }
    ...
    private static boolean toleranceSatisfied(double xip1, double xi, double delta) {
        double term1 = Math.abs(function(function, xip1) - function(function, xi));
        double term2 = delta * Math.abs(function(function, xip1));

        return (term1 <= term2);
    }

    private static void printSecantRowFunction(String function, int n, double xi, double xip1, double err) {
        String err_text = "";
        if (n == 0) {
            System.out.println(
                    padRight("n", 4) +
